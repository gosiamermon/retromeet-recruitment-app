...
import {
  ...
  USER_SET_SETTINGS_IN_PROGRESS, USER_SET_SETTINGS_SUCCESS, USER_SET_SETTINGS_FAILURE
} from '../actions/user';
import {
...
export const USER_SETTINGS_KEY = 'settings'
...
export const USER_SET_SETTINGS_QUERY_KEY = 'connectQuery';
...
const initialState = {
  ...
  [USER_SET_SETTINGS_QUERY_KEY]: QUERY_DEFAULT()
};
...
const ACTION_HANDLERS = {
  ...
  ...handleQuery([
    USER_SET_SETTINGS_IN_PROGRESS,
    {
      [USER_SET_SETTINGS_SUCCESS](state, payload) {
        const newState = deepClone(state);
        const { settings, id } = payload;

        if (newState[USER_ID_KEY] === id) newState[USER_SETTINGS_KEY] = settings;

        return newState;
      }
    },
    USER_SET_SETTINGS_FAILURE
  ], USER_SET_SETTINGS_QUERY_KEY),
